...
class RouteDefinitionBuilder {
	...
	fun <T : Handler> get(path: String, handler: T, vararg roles: FilterRole) = apply {
		routes.add(RouteDefinition(path, HandlerType.GET, handler, arrayOf(*roles)))
	}

	fun <T : Handler> post(path: String, handler: T, vararg roles: FilterRole) = apply {
		routes.add(RouteDefinition(path, HandlerType.POST, handler, arrayOf(*roles)))
	}

	fun <T : Handler> put(path: String, handler: T, vararg roles: FilterRole) = apply {
		routes.add(RouteDefinition(path, HandlerType.PUT, handler, arrayOf(*roles)))
	}

	fun <T : Handler> patch(path: String, handler: T, vararg roles: FilterRole) = apply {
		routes.add(RouteDefinition(path, HandlerType.PATCH, handler, arrayOf(*roles)))
	}

	fun <T : Handler> delete(path: String, handler: T, vararg roles: FilterRole) = apply {
